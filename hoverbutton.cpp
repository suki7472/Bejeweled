...
void HoverButton::setSound(QString pathHover, QString pathLeave, QString pathPress, QString pathRelease){
    //设置覆盖、按压、释放、离开音效
    if(pathHover != QString("")) {
        soundHover = new QSoundEffect(this);
        soundHover->setSource(pathHover.startsWith(":") ? QUrl("qrc" + pathHover) : QUrl::fromLocalFile(pathHover));
    }
    if(pathPress != QString("")) {
        soundPress = new QSoundEffect(this);
        soundPress->setSource(pathPress.startsWith(":") ? QUrl("qrc" + pathPress) : QUrl::fromLocalFile(pathPress));
    }
    if(pathRelease != QString("")) {
        soundRelease = new QSoundEffect(this);
        soundRelease->setSource(pathRelease.startsWith(":") ? QUrl("qrc" + pathRelease) : QUrl::fromLocalFile(pathRelease));
    }
    if(pathLeave != QString("")) {
        soundLeave = new QSoundEffect(this);
        soundLeave->setSource(pathLeave.startsWith(":") ? QUrl("qrc" + pathLeave) : QUrl::fromLocalFile(pathLeave));
    }
}
